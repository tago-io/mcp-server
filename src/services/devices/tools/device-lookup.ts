...
const deviceLookupSchema = querySchema.extend({
  ...
  include_configuration_params: z
    .boolean()
    .describe("If true, includes the configuration parameters for each device in the response. Default: false.")
    .optional(),
...
const deviceBaseSchema = z.object({
  operation: z.enum(["lookup", "delete", "create", "update"])
...
interface DeviceOperationHandler {
  lookup(params: DeviceSchema): Promise<string>;
  create(params: DeviceSchema): Promise<string>;
  update(params: DeviceSchema): Promise<string>;
  delete(params: DeviceSchema): Promise<string>;
}

function createDeviceLookupHandler(resources: Resources): Pick<DeviceOperationHandler, 'lookup'> {
  const getDeviceInfo = async (deviceID: string): Promise<string> => {
    const result = await resources.devices.info(deviceID);
    return convertJSONToMarkdown(result);
  };

  const listDevices = async (lookupDevice: DeviceSchema['lookupDevice']): Promise<string> => {
  ...
  return {
    async lookup(params: DeviceSchema): Promise<string> {
      const { deviceID, lookupDevice } = params;

      return deviceID 
        ? getDeviceInfo(deviceID)
        : listDevices(lookupDevice);
    }
  };
}

function createDeviceCreateHandler(resources: Resources): Pick<DeviceOperationHandler, 'create'> {
  return {
    async create(params: DeviceSchema): Promise<string> {
...
function createDeviceUpdateHandler(resources: Resources): Pick<DeviceOperationHandler, 'update'> {
  return {
    async update(params: DeviceSchema): Promise<string> {
...
function createDeviceDeleteHandler(resources: Resources): Pick<DeviceOperationHandler, 'delete'> {
  return {
    async delete(params: DeviceSchema): Promise<string> {
...
function createDeviceOperationHandler(operation: DeviceSchema['operation'], resources: Resources): DeviceOperationHandler {
  const handlers = {
    lookup: () => createDeviceLookupHandler(resources),
    create: () => createDeviceCreateHandler(resources),
    update: () => createDeviceUpdateHandler(resources),
    delete: () => createDeviceDeleteHandler(resources)
  };
  ...
  // Create a complete handler by combining all operation handlers
  const lookupHandler = createDeviceLookupHandler(resources);
  const createHandler = createDeviceCreateHandler(resources);
  const updateHandler = createDeviceUpdateHandler(resources);
  const deleteHandler = createDeviceDeleteHandler(resources);
...
const deviceOperationExecutors = {
  lookup: async (handler: DeviceOperationHandler, params: DeviceSchema) => {
    return handler.lookup(params);
  },
  create: async (handler: DeviceOperationHandler, params: DeviceSchema) => {
    return handler.create(params);
  },
  update: async (handler: DeviceOperationHandler, params: DeviceSchema) => {
    return handler.update(params);
  },
  delete: async (handler: DeviceOperationHandler, params: DeviceSchema) => {
...
async function deviceOperationsTool(resources: Resources, params: DeviceSchema) {
  ...
  const handler = createDeviceOperationHandler(operation, resources);
  
  return deviceOperationExecutors[operation](handler, validatedParams);
}
