...
async function analysisOperationsTool(resources: Resources, params: AnalysisSchema) {
  ...
  switch (operation) {
    case "lookup": {
      if (analysisID) {
        const result = await resources.analysis.info(analysisID as string);
        const markdownResponse = convertJSONToMarkdown(result);
        return markdownResponse;
      }
      const validatedQuery = validateAnalysisQuery(validatedParams.lookupAnalysis);
...
const analysisLookupConfigJSON: IDeviceToolConfig = {
  ...
  description: `Perform operations on analyses. It can be used to create, update, list and delete analyses.
